...
void FSLAMFilter::resetPosition(robot_pose_fslam::ResetPosition::Request& request, robot_pose_fslam::ResetPosition::Response& response){
	sem_wait(&mtx);
	...
	for (int i = 0; i < mcpdf->NumSamplesGet(); i++) {
		...
		stateIter->transform = request.pose
